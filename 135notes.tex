...
\[\input{noregion.pdf_tex}\]
% \drawing{80pt}{image.png}
...
% \drawing{80pt}{image-1.png}
{\def\svgwidth{80pt}
\[\input{annulus.pdf_tex}\]}
...
\[\input{smooth.pdf_tex}\]
% \drawing{150pt}{image-4.png}
...
\begin{figure}[htbp]
    \centering
    \input{uniform_convergence.svg.pdf_tex}
...
\begin{figure}[htbp]
    \centering
    \input{pointwise_convergence.svg.pdf_tex}
...
\sidenote{\def\svgwidth{\linewidth}\[\input{Riemann_surface_log.svg.pdf_tex}\]}
% \sidenote{See \href{https://stephangarcia.sites.pomona.edu/teaching/24S-135/Lecture/24S-135-Lecture10.pdf}{graphed Riemann surface}}
...
\eg $z^{1/2}$ would live on a surface:\sidenote{\def\svgwidth{\linewidth}\[\input{Riemann_surface_sqrt.svg.pdf_tex}\]}
\[\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
    %uncomment if require: \path (0,300); %set diagram left start at 0, and has height of 300
...
\begin{proof}[Proof gist]
    For a gist of why this works: Let $R_{1}<r_{1}<\rho_{1}<r<\rho_{2}<r_{2}<R_{2}$.\sidenote{For rigorous proof, see \href{https://stephangarcia.sites.pomona.edu/teaching/24S-135/Lecture/24S-135-Lecture17.pdf}{notes}.} \[\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
        %uncomment if require: \path (0,300); %set diagram left start at 0, and has height of 300
...
\begin{theorem}[Residue, simple vers.] Let $f:\Omega\to \C$ analytic except on the isolated singularity $z_0$. Then:\[\frac{1}{2\pi i}\int_{\gamma}f(\zeta)\,d\zeta=\Res(f; z_{0}) \]
...
\end{theorem}
\rmk[Lagaria's] At least 60\% of the zeros in the critical strip are on $\re s=\frac{1}{2}$. \sidenote{More about the zeros see \href{https://www.ias.edu/ideas/2013/primes-random-matrices}{here}.}

\subsubsection{Goldbach conjecture}
\conjecture[Goldbach] Every integer $\geq 6$ can be written as a sum of two odd primes.\sidenote{Still open since 1742!}
...
\addlink{Dirichlet's series}
\defn[Dirichlet's series] Series in the form of \[\sum_{n=1}^{\infty} \frac{a_n}{n^s}\]
...
Recall PNT said \[\pi(x)\sim \frac{x}{\log x} \sim \operatorname{Li}(x)\] where $\operatorname{Li}(x)=\int_{2}^{x}\frac{\d t}{\log t}$\sidenote{the \textit{offset} logarithmic integral (so that we avoid 0)} and $f\sim g$ means $\lim_{x\to\infty}\frac{f(x)}{g(x)}=1$.
...
Let $\Z[x]$ be the set of polynomials in $x$ with integer coefficients.\sidenote{not meaningful to talk about noninteger coefficient polys -- they don't assume primes anyway}
...
What conditions must $f$ satisfy if we want $Q(f;x)\to \infty$ as $x\to \infty$? \begin{itemize}
    ...
    \item $f$ should be irreducible over $\Z$.\sidenote{if the poly factors, then all the values also factor!}
...
\addlink{Bateman Horn conjecture}
\conjecture[Bateman-Horn] Let $f_1,f_2,\dots,f_k\in \Z[x]$ be distinct irreducible, with positive leading coefficients and are nonconstant.\sidenote{The truth of this conjecture would imply Goldbach!} Define 
% \tbd{}
